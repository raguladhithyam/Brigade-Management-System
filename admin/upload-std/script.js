...
function uploadVenueFile() {
    ...
    reader.onload = function (e) {
        ...
        for (let i = 0; i < rows.length; i++) {
            let columns = rows[i];
            addVenuePromises.push(addVenue(columns[0], columns[1],columns[2]));
        }
        ...
        Promise.all(addVenuePromises).then(() => {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Students Data Added',
                text: `${venuesCount} students have been added successfully!`,
...
function addVenue(stdroll, stdname, stdbg) {
    if (!stdroll) {
        console.error("Student details are empty.");
        return Promise.resolve();
    }

    // Convert stdroll to a string to ensure it's used correctly as a document ID
    let rollString = String(stdroll);

    return db.collection("students")
        .doc(rollString) // Use the string version of the roll number here
        .set({
            stdroll: rollString, // Optional: you can keep stdroll as a string in the database too
            stdname,
            stdbg,
        })
        .then(() => {
